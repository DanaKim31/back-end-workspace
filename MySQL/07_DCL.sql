...
CREATE USER test2@localhost IDENTIFIED BY 'test2'; -- @localhost : 해당 로컬 컴퓨터

-- 사용자 계정 삭제
DROP USER test;
DROP USER test2@localhost;

-- 현재 사용자에게 부여된 권한 목록 출력 (sys 데이터베이스 한해서) -> TEST 커넥션(test사용자) 에서는 계정 생성 불가
SHOW PRIVILEGES;
...
GRANT CREATE ON kh.* TO test; -- >> select 접근 안됨
