/*
    ...
    - 테이블에 새로운 행을 추가하는 구문
        ...
        3) INSERT INTO 테이블명 서브쿼리;
			- VALUES 대신 서브쿼리 조회한 결과값이 통째로 INSERT
...
-- 1. emp_dept 테이블에 employee에서 부서 코드가 D1인 직원의 사번, 이름, 부서코드, 입사일 추가
-- 2. emp_manager 테이블에 employee에서 부서 코드가 D1인 직원의 사번, 이름, 관리자 사번을 추가
...
-- 2.
-- ALTER TABLE emp_manager ADD manager_id VARCHAR(3);
...
/*
    ...
    - WHERE 절을 생락하면 모든 행의 데이터가 변경된다. (MySQL 방지)
...
SET emp_id = null
WHERE emp_id = 200;
-- > 해당 컬럼에 대한 제약조건이 위배되면 수정 안됨 > emp_id의 제약조건 : NOT NULL 이기 때문에 null 값으로 업데이트(수정) 할 수 없음
...
SELECT * FROM emp_salary WHERE bonus = 0.3;
...
ROLLBACK; -- 데이터 값 복구 (DELETE는 ROLLBACK 가능!)

TRUNCATE TABLE dept_copy;
TRUNCATE TABLE emp_salary;

ROLLBACK; -- 데이터 값 복구 안됨 (TRUNCATE는 ROLLBACK 불가능!) 

DROP TABLE dept_copy;
DROP TABLE emp_salary;

ROLLBACK; -- 데이터 값 복구 안됨 (DROP은 ROLLBACK 불가능!) 

-- >> ROLLBACK : INSERT, UPDATE, DELETE만 가능
