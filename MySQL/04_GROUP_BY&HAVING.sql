/*
    ...
    - select에는 group by에 지정할 컬럼과 그룹함수만 가능
*/
-- 각 부서별 조회
SELECT dept_code, 
	   count(*) "사원 수", 
...
-- 성별 별 사원 수 조회
SELECT if(substr(emp_no, -7, 1) = 1, '남', '여') "성별", count(*) "사원 수"
FROM employee
GROUP BY 성별;
...
/*
	...
	5 SELECT	* | 조회하고자 하는 컬럼 as 별칭 | 함수 
    1 FROM		조회하고자 하는 테이블명
    2 WHERE		조건식 (연산자들 가지고 기술, GROUP BY와 쓰일 때 그룹함수는 사용 못 함)
    3 GROUP BY	그룹 기준에 해당하는 컬럼명 | 함수 | 별칭
    4 HAVING	조건식 (그룹 함수를 가지고 기술) -> 그룹이 잡히고 나서 그룹에 대한 조건
    6 ORDER BY	컬럼명 | 컬럼순번 | 별칭 [ASC|DESC];
*/
-- 부서별 평균 급여가 300만원 이상인 부서의 평균 급여 조회
SELECT dept_code "부서", format(avg(ifnull(salary, 0)), 0) "평균급여"
FROM employee
GROUP BY dept_code
...
SELECT job_code "직급", format(sum(salary), 0) "급여의 합"
FROM employee
GROUP BY job_code
HAVING sum(salary) >= 10000000;

-- 부서별 보너스를 받는 사원이 없는 부서만 조회
SELECT dept_code "부서", count(bonus)
FROM employee
GROUP BY dept_code
...
/*
    ...
    UNION ALL (합집합) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출 (중복되는 행 제거하지 않음)
    INTERSEC (교집합) : 두 쿼리문을 수행한 결과값에 중복된 결과값만 추출 (MySQL X)
