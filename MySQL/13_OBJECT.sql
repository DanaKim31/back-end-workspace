...
/*
	프로시저(PROCEDURE)
    - SQL문을 저장하여 필요할 때마다 다시 입력할 필요없이 간단하게 호출해서 사용 가능하게 하는 코드 블록     
...
-- 프로시저 실행
CALL del_all_emp(); -- >> 에러 (매개변수가 들어가야 함)
CALL del_all_emp(200); -- >> emp_id가 200 인 사람 삭제
CALL del_all_emp(203); -- >> emp_id가 203 인 사람 삭제
CALL del_all_emp(201, '노옹철'); -- >> 201번은 송종기 이기 떄문에 에러 발생
CALL del_all_emp(201, '송종기'); -- >> 201번은 송종기 이기 떄문에 에러 발생
...
DELIMITER //
CREATE FUNCTION salary_calc(id INT) -- 변수로 쓸 이름과 유형 입력
RETURNS INT
DETERMINISTIC
BEGIN
	DECLARE total INT;
...
-- 방금 만든 salary_calc 함수를 이용해서 연봉이 4000만원 이상인 사원의 사번, 사원명, 급여, 연봉 조회(연봉 높은 순 정렬)
...
/*
    ...
    ex)
    - 회원 탈퇴 시 기존 회원 테이블에 데이터 DELETE 후 곧바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT 처리
    - 신고횟수가 일정 수를 넘었을 때 해당 회원을 블랙 리스트로 처리
...
FOR EACH ROW -- 각 행마다 확인해서 처리
BEGIN
	INSERT INTO log(event_type, event_desc) VALUES('INSERT', '신입사원이 입사했습니다');
END //
...
DROP TRIGGER IF EXISTS trg_02; -- 트리거 삭제
...
FOR EACH ROW
BEGIN
	INSERT INTO log(event_type, event_desc) VALUES('UPDATE', concat('변경 전 : ', OLD.dept_code, '변경 후 : ', NEW.dept_code));
END //

DELIMITER ;

-- employee 테이블에서 부서 코드가 D6인 직원들의 부서코드를 D3로 변경
UPDATE employee 
SET dept_code = 'D3'
WHERE dept_code = 'D6';
...
-- 샘플 데이터 3개 추가
INSERT INTO tb_product(pname, brand, price) VALUES('갤럭시24', '삼성', 1280000);
INSERT INTO tb_product(pname, brand, price, stock) VALUES('갤럭시Z 플립4', '삼성', 1000000, 10);
INSERT INTO tb_product(pname, brand, price, stock) VALUES('아이폰15', '애플', 1200000, 20);
