...
AS SELECT * FROM kh.department; -- 서브쿼리를 실제 쓸 일은 거의 없음
...
/*
	1-2. 컬럼 수정 (MODIFY)
    - 자료형(데이터 타입) 변경 : ALTER TABLE 테이블명 MODIFY 컬럼명 변경할자료형;
    - 기본값 변경 : ALTER TABLE 테이블명 MODIFY 컬럼명 자료형 DEFAULT 변경할기본값;
    
    * 참고 : DEFAULT 삭제
    ALTER TABLE 테이블명 MODIFY 컬럼명 DEFAULT NULL;
*/
-- dept_id 컬럼의 자료형을 CHAR(2)에서 CHAR(3)으로 변경
ALTER TABLE dept_copy MODIFY dept_id CHAR(3);

ALTER TABLE dept_copy MODIFY dept_id INT; -- 에러 발생! 기존 데이터가 문자열로 있기 때문에 오류 발생
ALTER TABLE dept_copy MODIFY cname INT; -- 실행 가능! 기존 데이터가 없으면 자료형 변경이 가능

-- dept_title의 자료형을 VARCHAR(5)로 변경
ALTER TABLE dept_copy MODIFY dept_title VARCHAR(5); -- > 변경하려는 자료형의 크기보다 이미 큰 값이 존재하면 에러 발생!
...
ALTER TABLE dept_copy 
    ...
    -- > 위에서 '한국'으로 지정했기 때문에 NULL이 아님. 자료형은 '미국'으로 변경되었으나 테이블에서는 '한국'으로 유지
...
/*
    ...
    - 데이터 값이 기록되어 있어도 같이 삭제된다. (삭제하려는 컬럼과 해당 컬럼의 데이터 전체 삭제됨)
    - 삭제된 컬럼 복구는 불가능
...
-- > 참조되고 있는 컬럼이 있어서 삭제가 불가능 (참조되고 있는 컬럼 : 하단 member 테이블 내)
...
ALTER TABLE dept_copy ADD CONSTRAINT dept_copy_dept_id_pk PRIMARY KEY(dept_id); -- 제약조건명 추가한 버전
...
-- 어딘가에서 참조되고 있는 부모 테이블은 함부로 삭제가 안됨!
