/* 
	...
	- SQL 종류
		1. DML(Data Manipulation Language) : 데이터 조작어
			- 실제 데이터를 조작하기 위해 사용하는 언어 (INSERT : 추가, UPDATE : 수정, DELETE : 삭제, SELECT : 조회)
		...
		4. DDL(Data Definition Language) : 데이터 정의어
			- DBMS의 구조를 정의하거나 변경, 삭제하기 위해 사용하는 언어
              (CREATE : 생성, ALTER : 수정, DROP : 삭제) - 테이블 변경(DML은 테이블 내 데이터 조작)
*/
...
-- 2. 부서코드가 'D1'이 아닌 사원들의 사번(emp_id), 사원명, 부서코드 조회
...
/*
	BETWEEN AND
    - 조건식에서 사용되는 구문
    - 몇 이상 몇 이하의 범위에 대한 조건을 제시할 때 사용되는 연산자
...
/*
    ...
    '%' : 0글자 이상
    비교대상컬럼 LIKE '문자%' => 비교대상컬럽 값이 문자로 "시작"되는걸 조회
    비교대상컬럼 LIKE '%문자' => 비교대상컬럼 값이 문자로 "끝"나는걸 조회
    비교대상컬럼 LIKE '%문자%' => 비교대상컬럼 값이 문자가 "포함"되는걸 조회(키워드 검색)
    
    '_' : 1글자
    비교대상컬럼 LIKE '_문자' => 비교대상컬럼 값에 문자 앞에 무조건 한글자가 올 경우 조회
    비교대상컬럼 LIKE '__문자' => 비교대상컬럼 값에 문자 앞에 무조건 두글자가 올 경우 조회
    비교대상컬럼 LIKE '_문자_' => 비교대상컬럼 값에 문자 앞과 뒤에 무조건 한글자가 올 경우 조회
*/
...
-- 사원들 중 이름에 '하'가 포함된 사원들의 사원명, 주민번호(emp_no), 전화번호(phone) 조회
...
*/ 
-- 보너스를 받지않는 사원(즉, bonus 값이 NULL)들의 사번, 이름, 급여, 보너스 조회
...
-- 5. 급여가 200만원 이상이고 500만원 이하고 입사일이 '2000-01-01' 이상이고 보너스를 받지 않는 사원들의 사번, 사원명, 급여, 입사일, 보너스 조회
SELECT emp_id, emp_name, salary, hire_date, bonus
FROM employee
WHERE salary >= 2000000 AND hire_date >= '2000-01-01' AND bonus IS NULL;

-- 6. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사원들의 사번, 사원명, 급여, 보너스 포함 연봉 조회(별칭 사용)
