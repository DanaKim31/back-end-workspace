...
/*
    ...
    * 자료형 (MySQL 기준)
    1. 문자 (VARCHAR 많이 사용)
		- CHAR / **VARCHAR** : 고정(CHAR) 및 가변(VARCHAR) 길이 문자, 반드시 크기 지정
        - TEXT : 매우 긴 문자열을 저장하는데 사용
	2. 숫자 (INT 많이 사용)
		- **INT** : 정수값 저장하는데 사용
        - FLOAT / DOUBLE : 부동소수점 저장하는데 사용
        - DECIMAL : 고정 소수점 저장하는데 사용
	3. 날짜 및 시간 (DATE 많이 사용)
		- **DATE** : 날짜 저장하는데 사용
        - TIME : 시간 저장하는데 사용
        - DATETIME / TIMESTAMP : 날짜와 시간을 함께 저장
	4. 불리언
		- BOOLEAN / BOOL : 참(True) 도는 거짓(False) 값을 저장하는데 사용
	5. 이진 자료형
		- BLOB : 이진 데이터를 저장하는데 사용. 이미지나 동영상과 같은 이진 파일
        --> 실제로는 이미지나 동영상은 데이터베이스에 저장하지 않고 따로 관리함. 데이터베이스에는 주소(문자)로 저장
*/
...
-- 테이블에 데이터를 추가 시키는 구문 (DML : INSERT) --> 뒤에서 배울 예정
...
INSERT INTO mem_notnull VALUES(2, 'user02', null, '김현수', '남', null, NULL, null);
-- > NOT NULL 제약조건에 위배되어 오류 발생! (비밀번호 컬럼에 null을 입력했기 때문)
...
-- > 기본키(PRIMARY KEY)에 중복값을 담으려고 해서 문제 발생! (unique 제약조건 위배)
-- > 기본키(PRIMARY KEY)를 mem_no + mem_id 복합키로 설정했을 때는 가능!

INSERT INTO mem_pri VALUES(null, 'user02', 'pass02', '조세미', '여', null, null, null);
-- > 기본키(PRIMARY KEY)에 null을 담으려고 해서 문제 발생! (not null 제약조건 위배)
...
-- 두 테이블 간 공통되는 컬럼이 필요할 경우 사용(찜, 좋아요, 댓글, 주문 ...)
DROP TABLE tb_like;
...
-- > 복합기 방식으로 지정해서 쓰는거 추천 안함!

-- 회원등급에 대한 데이터를 따로 보관하는 테이블 생성
...
WHERE grade_code = 10; 
-- > 자식테이블(member)에서 10 등급을 사용하고 있기 때문에 삭제 안 됨!

-- mem_grade 테이블에서 grade_code가 30인 등급 삭제
DELETE FROM mem_grade
WHERE grade_code = 30;
-- > 자식테이블(member)에서 30 등급을 사용하지 않고 있기 때문에 삭제 가능!
...
/*
    ...
    - 삭제옵션 : 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 처리할 건지에 대한 옵션
    
    1. ON DELETE RESTRICTED (기본값) : 자식데이터로 쓰이는 부모데이터는 삭제가 아예 안되게끔 
	2. ON DELETE SET NULL : 부모데이터 삭제 시 해당 데이터를 쓰고 있는 자식 데이터의 값을 NULL로 처리
...
SELECT * FROM mem_grade; -- 10 등급 행 삭제 됨
SELECT * FROM member; -- 10등급을 사용하던 멤버 행 자체가 삭제되어 출력됨

/*
	DEFAULT 기본값
    - 제약조건 아님!
    - 컬럼을 선정하지 않고 INSERT 시 NULL이 아닌 기본값을 세팅해 주는 값
*/
DROP TABLE member;
CREATE TABLE member( 
...
-- > 선택되지 않은 컬럼에는 기본적으로 NULL이 들어감. 단, 해당 컬럼에 DEFAULT 값이 부여되어 있을 경우 NULL이 아닌 DEFAULT 값이 들어감
...
/*
    ...
    AS 서브쿼리;
*/
-- employee 테이블 복사하여 새로운 테이블 생성(커럼, 데이터, 테이터타입, not null 제약조건 전부 복사)
...
-- 모든 행에 대해서 매번 false이기 때문에 테이블의 구조만 복사되고 데이터 값은 복사되지 않는다.
