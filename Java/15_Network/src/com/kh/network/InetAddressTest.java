 ...
 * 	- xxx.xxx.xxx.xxx와 같은 형식으로 표현 (xxx는 부호없는 0 ~ 255 사이의 정수)
 * 	- DNS(Domain Name System)를 통해 해당 도메인의 IP 주소를 알아낼 수 있다
 * 
 * 포트(Port)
 * 	- IP주소는 네트워크 상에서 컴퓨터를 찾을 때만 사용하고, 컴퓨터 내에서 실행하는 프로그램을 선택하기 위해 포트 사용
 * 	- 클라이언트는 서버 연결 요청(모든 네트워크의 시작은 클라이언트의 요청으로 시작) 시 IP주소와 함께 포트 번호를 알고 있어야 한다
