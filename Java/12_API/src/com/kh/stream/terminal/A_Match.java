...
/*
 * 매칭
 * 	- 최종처리 단계에서 요소들이 특정 조건에 만족하는지 조사하는 역할
...
//		a.method1();
	...
	public void method1() {
		int[] values = {2, 4, 6};
		boolean result = Arrays.stream(values).allMatch(value -> value % 2 == 0); // allMatch 리턴타입 : boolean
		System.out.println("짝수만 가지고 있는가? " + result); // true
		
		result = Arrays.stream(values).anyMatch(value -> value % 3 == 0); // 배열 중 하나라도 3의 배수가 있는가
	...
	public void method2() {
		...
		boolean result = students.stream().allMatch(student -> student.getAge() >= 25 && student.getGender() == "남자");
		System.out.println("나이가 25살 이상인 학생들이 모두 남자인지? " + result);
		
		// 남학생들 중에 평균이 70점 이상인 학생이 한 명이라도 존재하는지 확인
		result = students.stream().anyMatch(student -> (student.getMath() + student.getEnglish()) / 2 >= 70 && student.getGender().equals("남자"));
