...
public class Application {
	...
	public void mainMenu() {
		 ...
		 * "1. 새 회원 등록" 하지 못하게 -> 화면상 안보이게만 처리해도 됨
		 * 
		 * 1번인 경우 -> insertMember()
		 * 2번인 경우 -> updateMember()
		 * 3번인 경우 -> printAll()
		 * 9번인 경우 -> 프로그램 종료
		 * 그 외의 경우 -> 잘못 입력하셨습니다. 다시 입력해 주세요.
		 */
		boolean check = true;
		while(check) {
		System.out.println("최대 등록 가능한 회원 수는 3명입니다.\n현재 등록된 회원수는 " + mc.count + "명입니다.");
		
			if(mc.count < 3) System.out.println("1. 새회원 등록");
			...
			try {
				switch(Integer.parseInt(sc.nextLine())) { // UnChekedException이기 때문에 try 안으로 넣기
				case 1 : if(mc.count < 3) insertMember();
					break;
				case 2 : updateMember();
					break;
				case 3 : printAll();
			...
			} catch(Exception e) { // NumberFormatException이지만 Exception으로도 처리 가능
...
//		Member m = new Member();
		...
		try { // MemberController에서 throw 후 오류 발생 > try~catch 추가
			if(mc.checkId(id) != -1) {
	...
	//			 매개변수로 Member 변수 통째로 넘기기 > Controller에서도 통째로 받기
				Member m = new Member(id, name, password, email, gender, age);
				mc.insertMember(m);
			}
		} catch (DuplicateNameException e) {
			e.printStackTrace(); // 기존 회원이 있을 때 에러 출력
