...
public class A_TryCatch {
	 ...
	 * 런타임 에러(runtime error) : 코드 실행은 되나 실행 시에 발생하는 에러 
	 ...
	 *  
	 *  Object -> Throwable -> Exception(클래스) / Error (상속관계)
	 *  
	 ...
	 * 
	 * try ~ catch - try{} catch(){} : 필수입력, finally{} : 생략가능
	 * 
	 * try {
	 * 
	 * 		// 예외가 발생할 가능성이 있는 코드들
	 ...
	 * } [finally {
	 * 	
	 * 		// 예외가 발생하거나 발생하지 않아도 무조건 실행되는 코드들
	...
	public static void main(String[] args) {
		...
		try {
			...
			throw new Exception(); // 고의로 예외 발생! try catch 없이 작성 시 컴파일 에러 발생
		
		} catch(Exception e) {
			
			// e.printStackTrace(); // 오류 메시지
			System.out.println(e.getMessage()); // null
			
			// printStackTrace() : 예외발생 당시의 호출스택(Call Stack)에 있었던 메서드의 정보와 예외 메시지를 화면에 출력
			// get.Message() : 발생한 예외클래스의 객체에 저당된 메시지를 얻을 수 있음
