...
public class C_For {
	 ...
	 * - 초기식 : 반복문이 수행될 때 단 한 번만 실행, 반복문 안에서 사용할 변수를 선언하고 초기갑을 대입
	...
	public void method1() {
		 ...
		 * 4. i++
		 * -> 4번 실행 후 2번 ~ 4번 재실행, 4번에서 i가 6이 될 경우 2번에서 false 이므로 반복문을 빠져나감
		 */
		for(int i=1; i<=5; i++) {
	...
	// 1~10 사이의 숫자 중 짝수만 출력
	public void method4() {
		for(int i=1; i<=10; i++) {
			if(i % 2 != 0) continue; // i가 홀수인 경우는 건너 뛰겠다(continue)
			System.out.println(i);
		}
	}
	...
	public void method8() {
		 ...
		 * - String.toCharArray() : 모든 문자가 들어있는 char[] 형식의 데이터를 반환 / [] : 배열을 의미
		 * */
