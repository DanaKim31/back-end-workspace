...
public class Card {
	 ...
	 * 		- 각 인스턴스의 개별적인 저장공간
	 * 		- 인스턴스 생성 후, '참조변수.인스턴스변수명' 으로 접근 (객체 생성 후에 가능)
	 * 		- 인스턴스를 생성할 때 생성되고, 참조변수가 없을 때 가비지컬렉터에 의해 자동 제거
	 * 
	 * 3. 클래스변수(class variable) -> static(static을 붙이면 프로그램이 종료될 때까지 계속 쌓임 > 용량 차지 많이해서 많이 사용하지 않음)
	 * 		- 같은 클래스의 모든 인스턴스들이 공유하는 변수
	 * 		- 인스턴스 생성 없이 '클래스이름.클래스변수명' 으로 접근 (객체 생성없이도 가능)
